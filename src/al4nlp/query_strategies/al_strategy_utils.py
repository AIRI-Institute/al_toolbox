...
def get_query_idx_for_selecting_by_number_of_tokens(
    data: Union[
        List[Union[str, List[str]]], np.ndarray, ArrowDataset, TransformersDataset,
    ],
...
def calculate_mnlp_scores_empty_penalty(probas) -> np.ndarray:
    positiveness = np.max(np.argmax(probas, axis=-1), axis=-1) != 0
    scores = np.array([-np.sum(np.log(np.max(i, axis=1))) / len(i) for i in probas])
    return np.where(positiveness, scores, -np.inf)
...
def calculate_alps_scores(
    ...
    **tokenization_kwargs,
    ...
    device = next(model_wr.model.parameters()).device
    if device.type == "cuda":
        model_wr.model.cpu()
        torch.cuda.empty_cache()
    model = AutoModelForMaskedLM.from_pretrained(model_wr.model_config.checkpoint).to(
        device
    )
    ...
    model_wr.tokenizer.model_max_length = 256
    ...
    if not isinstance(dataloader_or_data, DataLoader):
        if not data_is_tokenized:
            ...
            tokenize_function = wrapper.get_tokenize_function(**tokenization_kwargs,)
            columns_to_remove = [
                x
            ...
            if isinstance(wrapper, WrapperNer):
    ...
    # Return the original model to cuda
    if device.type == "cuda":
        torch.cuda.empty_cache()
        model_wr.model.to(device)
...
def calculate_bald_score_ner(sampled_probabilities) -> np.ndarray:
    # sampled_probabilities: list of num_samples (k) x batch_size (n) x num_tokens (l) x num_labels (c)
    bald_scores = []
    for i_inst in range(len(sampled_probabilities[0])):
        inst_runs = np.r_[[run[i_inst] for run in sampled_probabilities]]
        score = (
            entropy(np.mean(inst_runs, 0)).mean()
            - np.mean(entropy(inst_runs), 0).mean()
        )
...
def get_similarities(
    ...
    all_data = all_data.map(
        lambda x: tokenizer(x[text_name], truncation=True,),
        batched=True,
...
def calculate_unicentroid_mahalanobis_distance(embs_unique, labeled_indices):
    ...
    dists = mahalanobis_distance_with_known_centroids_sigma_inv(
        centroids, centroids_mask, sigma_inv, embs_unique,
    )
...
def calculate_mahalanobis_distance(
    ...
    if not batched:
        ...
        dists = func_for_dists_calc(
            centroids, centroids_mask, sigma_inv, eval_embeddings,
        )
        if use_triplet:
            closest_centroid_distance = dists.min(dim=1)[0]
            dists = (
...
def calculate_actune_scores(
    ...
    pseudo_classes = np.argmax(probas, axis=-1)
    ...
    km = KMeans(n_clusters=k).fit(test_embeddings)

    cluster_unc = []

    for clust_lbl in range(k):
        cluster_ids = np.where(km.labels_ == clust_lbl)[0]
        cluster_class = pseudo_classes[cluster_ids]
        cur_cluster_div = 0
        for i in np.unique(cluster_class):
            cur_ids = np.where(cluster_class == i)[0]
            freq = len(cur_ids) / len(cluster_class)
            cur_cluster_div += freq * np.log(freq)
        cluster_unc.append(
            np.sum(uncertainty_estimates[cluster_ids]) / len(cluster_ids)
            - lamb * cur_cluster_div
        )

    cl_argsort = np.argsort(np.array(cluster_unc))

    rez = []
    for i in cl_argsort:
        cluster_ids = np.where(km.labels_ == i)[0]
        samples_from_cluster = n_instances // k
        argsort = np.argsort(-uncertainty_estimates[cluster_ids])[:samples_from_cluster]
        rez.extend(cluster_ids[argsort])

    return np.array(rez), uncertainty_estimates
